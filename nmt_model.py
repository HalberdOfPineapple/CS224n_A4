...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X = self.model_embeddings.source(source_padded) # Tensor (src_len, b, e)
        X = pack_padded_sequence(X, torch.tensor(source_lengths))
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X) # all rnn modules accept PackedSequence as input
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        ###               as well as the new combined output o_t.a
        ...
        # torch.split with split_size_or_section = 1:
        # to split along the (default) first dimension with step 1
        # and return a tuple. The tensor in the tuple keeps first 1
        # in its dimension
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.squeeze(a_t, dim=1) # (b, 2 * h)
